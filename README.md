...
- **No external dependencies in development/test** — SQLite handles the primary database, job queue, cache, and ActionCable backend; no Redis or separate cache server required
...
- **Production uses four separate SQLite files** — primary, cache, queue, and cable databases each on a persistent Docker volume
...
- Ruby 3.4.2 (`rbenv` or `asdf` recommended)
- Bundler 2.x
...
`bin/dev` starts Rails via Foreman using `Procfile.dev`, which runs both the Rails server and the Tailwind CSS watcher in parallel.
...
# Run the full CI suite (security + lint + tests)
bin/rails test && bundle exec brakeman --no-pager && bundle exec rubocop
```

The test suite uses **Rails MiniTest** with **fixtures** for test data. Controller tests assert authentication enforcement, authorization boundaries (users can only access their own records), correct redirects, and HTTP status codes. Model tests cover validations, enums, scopes, and business logic methods.
...
In production, both secrets are injected via Kamal from `.kamal/secrets`. In development, store `ANTHROPIC_API_KEY` in `config/credentials.yml.enc`:
...
The application is deployed via **Kamal** to a single Docker host with an Nginx SSL proxy managed by Let's Encrypt.
...
The `bin/docker-entrypoint` script runs `db:prepare` on every container start, applying any pending migrations automatically.
...
Failed system test screenshots are automatically uploaded as GitHub Actions artifacts.
...
# Parse a natural-language food log
...
All pull requests run the full CI pipeline automatically. Screenshots from failed system tests are uploaded as artifacts.
